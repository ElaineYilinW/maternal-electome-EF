"""
dCSFA_NMF Model - Version 1.2 (Fixed)

Fixes:
1. Train/Val loss scale consistency (both record mean values)
2. phi_l2_loss computed once per epoch (not accumulated in batch loop)
3. get_sup_recon indexing bug fixed

Based on: Talbot et al. 2023 - SAE-NMF for brain network analysis

Add random_state=42 to skl_pretrain on Feb 04
"""
...
VERSION = 1.3  # Added early stopping with patience


class dCSFA_NMF(nn.Module):
    def __init__(self, n_components, dim_in, device='auto', n_intercepts=1, n_sup_networks=1,
                 optim_name='AdamW', recon_loss='IS', sup_recon_weight=1.0, sup_weight=1.0, phi_weight=1.0,
        ...
        - phi_l2_loss is now computed ONCE per epoch (consistent with paper Eq. 6)
        - Train/Val losses are both recorded as mean values (comparable)
        - get_sup_recon indexing fixed

        Inputs
        ----------------------------
        n_components - int - The number of latent factors to learn.
        dim_in - int - The number of features of the input.
        device - {'auto','cpu','cuda'} - The device that the model will be ran on.
        n_intercepts - int - The number of unique intercepts to use.
        n_sup_networks - int - The number of supervised networks you would like to learn.
        optim_name - {'AdamW','Adam','SGD'} - The optimizer being used.
        recon_loss - {'IS','MSE'} - Reconstruction loss.
        ...
        phi_weight - float - weight for phi L2 regularization (applied ONCE per epoch)
        useDeepEnc - bool - indicates if a neural network encoder should be used
        sup_recon_type - {"Residual","All"} - controls supervised reconstruction type
        fixed_corr - string / list - {"n/a","positive","negative"} - correlation constraint
        momentum - float - Momentum value if optim_name is "SGD".
        """
    ...
    def get_W_nmf(self):
        """Get normalized W_nmf (loadings) with L2 norm = sqrt(p) per factor"""
        W = nn.Softplus()(self.W_nmf)
        p = W.shape[1]
        W_normalized = F.normalize(W, p=2, dim=1) * np.sqrt(p)
        return W_normalized

    def get_phi(self, sup_net=0):
        """Get phi with optional sign constraint"""
    ...
    def _compute_phi_l2_loss(self):
        """
        Compute phi L2 regularization loss.
        This should be called ONCE per epoch, not per batch.
        
        Corresponds to loss(Φ) in paper Equation (6).
        """
        phi_l2 = 0
        for sup_net in range(self.n_sup_networks):
            phi = self.get_phi(sup_net)
            phi_l2 += torch.sum(phi ** 2)
        return self.phi_weight * phi_l2
    ...
    def skl_pretrain(self, X, y, nmf_max_iter=100):
        ...
        if self.recon_loss == "IS":
            skl_NMF = NMF(n_components=self.n_components, solver="mu", beta_loss="itakura-saito", init='nndsvda',
                          max_iter=nmf_max_iter, random_state=42)
        else:
            skl_NMF = NMF(n_components=self.n_components, max_iter=nmf_max_iter, random_state=42)
        ...
        for sup_net in range(self.n_sup_networks):
            ...
            print("Selecting network: {} with auc {} for sup net {} using constraint {} correlation".format(
                current_net, current_auc, sup_net, self.fixed_corr[sup_net]))
            selected_networks.append(current_net)
            selected_aucs.append(current_auc)
    ...
    def get_sup_recon(self, s):
        """Fixed: use :self.n_sup_networks instead of self.n_sup_networks"""
    ...
    def residual_loss_f(self, s, s_h):
        res_loss = torch.norm(s[:, :self.n_sup_networks].view(-1, self.n_sup_networks) - s_h) / (
    ...
    def forward(self, X, intercept_mask=None, avgIntercept=False):
        ...
        if self.sup_recon_type == "Residual":
            ...
            X_recon_sup = self.get_sup_recon(s)
            sup_recon_loss = self.recon_loss_f(X_recon_sup, X)
        else:
    ...
    def fit(self, X, y, y_pred_weights=None, y_sample_groups=None, intercept_mask=None, task_mask=None,
            n_epochs=100, n_pre_epochs=100, nmf_max_iter=100, batch_size=128, lr=1e-3,
            pretrain=True, verbose=False, print_rate=5,
            X_val=None, y_val=None, y_pred_weights_val=None, task_mask_val=None,
            best_model_name="dCSFA-NMF-best-model.pt",
            patience=None, min_delta=1e-4):
        """
        Fit the dCSFA-NMF model.
        
        VERSION 1.3 UPDATES:
        ------------------
        1. Added early stopping with patience
           - Stops when val_loss doesn't improve by min_delta for `patience` epochs
           - Set patience=None to disable (run all epochs, just do model selection)
        
        2. phi_l2_loss is computed ONCE per epoch (consistent with paper Eq. 6)
        
        3. Train/Val losses are recorded as MEAN values (directly comparable)
        
        Parameters
        ----------
        X : array-like, shape (n_samples, n_features)
            Training data
        y : array-like, shape (n_samples, n_sup_networks)
            Training labels
        ...
        y_val : array-like, optional
            Validation labels
        best_model_name : str
            Filename to save best model (when using validation)
        patience : int, optional
            Number of epochs to wait for improvement before stopping.
            If None, no early stopping (just model selection).
        min_delta : float, optional
            Minimum improvement in val_loss to be considered as improvement.
            Default: 1e-4
        """
        ...
        self.train_total_hist = []
        self.train_recon_hist = []
        self.train_pred_hist = []
        self.train_auc_hist = []
        self.train_phi_loss_hist = []
        ...
        # Get n_batches for phi_l2_loss scaling
        ...
        for epoch in epoch_iter:
            ...
            for X_batch, y_batch, b_mask_batch, task_mask_batch, y_pred_weight_batch in loader:
                ...
                # Prediction loss (mean over batch)
                pred_loss = self.sup_weight * nn.BCELoss(weight=y_pred_weight_batch)(
                    y_pred_proba * task_mask_batch, y_batch * task_mask_batch
                )

                # ========== FIXED: phi_l2_loss divided by n_batches ==========
                # Paper Eq. (6): loss(Φ) is OUTSIDE the sum over samples
                # By dividing by n_batches, the total phi_l2_loss over all batches
                # equals the single-epoch value (not accumulated n_batches times)
                phi_l2_loss = self._compute_phi_l2_loss() / n_batches

                # Total loss for this batch
                loss = recon_loss + pred_loss + phi_l2_loss
                loss.backward()
                optimizer.step()

                # Accumulate for logging (will divide by n_batches later)
                epoch_recon_loss += recon_loss.item()
                epoch_pred_loss += pred_loss.item()
                batch_count += 1

            # ========== Record MEAN values (consistent with validation) ==========
            mean_recon_loss = epoch_recon_loss / batch_count
            mean_pred_loss = epoch_pred_loss / batch_count
            
            # phi_l2_loss: compute once (not accumulated)
            with torch.no_grad():
                phi_l2_value = self._compute_phi_l2_loss().item()
            
            mean_total_loss = mean_recon_loss + mean_pred_loss + phi_l2_value
            
            self.train_total_hist.append(mean_total_loss)
            self.train_recon_hist.append(mean_recon_loss)
            self.train_pred_hist.append(mean_pred_loss)
            self.train_phi_loss_hist.append(phi_l2_value)
            ...
            with torch.no_grad():
                ...
                for sup_net in range(self.n_sup_networks):
                    training_sample_auc_list.append(
                        roc_auc_score(y.cpu().detach().numpy()[:, sup_net],
                                      y_pred_proba_train.cpu().detach().numpy()[:, sup_net])
                    )
                self.train_auc_hist.append(training_sample_auc_list)
            ...
            if X_val is not None and y_val is not None:
                with torch.no_grad():
                    ...
                    val_pred_loss = self.sup_weight * nn.BCELoss(weight=y_pred_weights_val)(
                        y_pred_proba_val * task_mask_val, y_val * task_mask_val
                    )

                    # phi_l2_loss for validation (same as training, computed once)
                    val_phi_l2_loss = self._compute_phi_l2_loss()

                    val_loss = val_recon_loss + val_pred_loss + val_phi_l2_loss

                    # Record validation losses (already mean values)
                    self.val_total_hist.append(val_loss.item())
                    self.val_recon_hist.append(val_recon_loss.item())
                    self.val_pred_hist.append(val_pred_loss.item())

                    # Validation AUC
                    val_sample_auc_list = []
                    for sup_net in range(self.n_sup_networks):
                        val_sample_auc_list.append(
                            roc_auc_score(y_val.cpu().detach().numpy()[:, sup_net],
                                          y_pred_proba_val.cpu().detach().numpy()[:, sup_net])
                        )
                    self.val_auc_hist.append(val_sample_auc_list)

                # Save best model and check early stopping
                if val_loss.item() < self.best_val_loss - self.min_delta:
                    # Significant improvement
                    self.best_epoch = epoch
                    self.best_val_recon = val_recon_loss.item()
                ...
                if self.patience is not None and self.patience_counter >= self.patience:
                    if verbose:
                        ...
                        print(f"    No improvement > {self.min_delta} for {self.patience} epochs")
            ...
            if verbose and (X_val is not None and y_val is not None):
                epoch_iter.set_description(
                    "Ep:{}, Best:{}, TrLoss:{:.4f}, ValLoss:{:.4f}, TrAUC:{:.3f}, ValAUC:{:.3f}".format(
                        epoch, self.best_epoch, mean_total_loss, val_loss.item(), 
                        training_sample_auc_list[0], val_sample_auc_list[0]
                    )
                )
                # Print detailed info every 100 epochs
                if epoch % 100 == 0 or epoch == n_epochs - 1:
                    print(f"\n  [Epoch {epoch}] Train: recon={mean_recon_loss:.4f}, pred={mean_pred_loss:.4f}, phi={phi_l2_value:.6f}")
                    print(f"             Val: recon={val_recon_loss.item():.4f}, pred={val_pred_loss.item():.4f}")
                    print(f"             Best epoch so far: {self.best_epoch}, Best val loss: {self.best_val_loss:.4f}")
            elif verbose:
                epoch_iter.set_description(
                    "Epoch: {}, Loss: {:.4f}, Recon: {:.4f}, AUC: {:.3f}".format(
                        epoch, mean_total_loss, training_mse_loss, training_sample_auc_list[0]
                    )
                )
        ...
        if X_val is not None and y_val is not None:
            ...
            try:
                ...
                # Older PyTorch versions don't support the weights_only argument
    ...
    def get_skl_mse_score(self, X, nmf_max_iter=500):
        if self.skl_pretrain_model is not None:
            ...
            if self.recon_loss == "IS":
                skl_NMF = NMF(n_components=self.n_components, solver="mu", beta_loss="itakura-saito", init='nndsvda', max_iter=100)
            else:
                skl_NMF = NMF(n_components=self.n_components, max_iter=100)
