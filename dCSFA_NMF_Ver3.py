"""
dCSFA_NMF Model - Version 1.3 (Fixed)
...
VERSION = 1.4  # Fix: eval()/train() toggle for validation; numerically safe inverse_softplus
...
class dCSFA_NMF(nn.Module):
    ...
    def fit(self, X, y, y_pred_weights=None, y_sample_groups=None, intercept_mask=None, task_mask=None,
        ...
        # ========== Post-pretrain diagnostic & sup_weight auto-adjustment ==========
        if pretrain:
            self.eval()
            with torch.no_grad():
                X_recon_diag, _, y_pred_diag, _ = self.forward(X, intercept_mask)
                
                if self.feature_groups is not None:
                    post_pretrain_recon = self.get_weighted_recon_loss_f(X_recon_diag, X).item()
                else:
                    post_pretrain_recon = self.recon_loss_f(X_recon_diag, X).item()
                
                post_pretrain_pred_raw = nn.BCELoss(weight=y_pred_weights)(
                    y_pred_diag * task_mask, y * task_mask
                ).item()
                post_pretrain_pred = self.sup_weight * post_pretrain_pred_raw
            self.train()
            
            self.post_pretrain_recon_loss_ = post_pretrain_recon
            self.post_pretrain_pred_loss_ = post_pretrain_pred
            self.sup_weight_original_ = self.sup_weight
            self.sup_weight_adjusted_ = False
            
            print(f"\n{'='*60}")
            print(f"POST-PRETRAIN DIAGNOSTIC")
            print(f"{'='*60}")
            print(f"  Recon loss:                    {post_pretrain_recon:.6f}")
            print(f"  Pred loss (sup_weight × BCE):  {post_pretrain_pred:.6f}")
            print(f"  Ratio (pred / recon):          {post_pretrain_pred / post_pretrain_recon:.4f}" 
                  if post_pretrain_recon > 0 else "  Ratio: recon_loss is 0")
            
            # Auto-adjust if pred_loss > 2.0 * recon_loss
            if post_pretrain_recon > 0 and post_pretrain_pred > 2.0 * post_pretrain_recon:
                new_sup_weight = (2.0 * post_pretrain_recon) / post_pretrain_pred_raw
                print(f"\n  ⚠ pred_loss ({post_pretrain_pred:.6f}) > 2.0 × recon_loss ({2.0*post_pretrain_recon:.6f})")
                print(f"  AUTO-ADJUSTING sup_weight: {self.sup_weight} → {new_sup_weight:.6f}")
                self.sup_weight = new_sup_weight
                self.sup_weight_adjusted_ = True
                
                new_pred = new_sup_weight * post_pretrain_pred_raw
                print(f"  New pred loss = {new_pred:.6f} = 2.0 × recon_loss")
            else:
                print(f"\n  ✓ pred_loss is within 2.0× recon_loss, no adjustment needed")
            print(f"  sup_weight for training: {self.sup_weight}")
            print(f"{'='*60}\n")
